...
public class SpaceSkyboxProcedure {
	...
	public static void renderStars(int amount, int seed, float yaw, float pitch, float roll, int color, boolean constant) {
		if (starBuffer == null || amount != SpaceSkyboxProcedure.amount || seed != SpaceSkyboxProcedure.seed) {
			SpaceSkyboxProcedure.amount = amount;
			SpaceSkyboxProcedure.seed = seed;
			RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
			RenderSystem.setShader(GameRenderer::getPositionShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().getBuilder();
	...
	public static void renderSky(TickEvent.RenderTickEvent event) {
		...
		try {
			...
			if (!effects.getName().contains("TardifModModDimensionEffects"))
				return;
			if (effects != SpaceSkyboxProcedure.effects) {
				SpaceSkyboxProcedure.effects = effects;
				SpaceSkyboxProcedure.customSky = (List<Predicate<Object[]>>) effects.getField("CUSTOM_SKY").get(null);
			}
			SpaceSkyboxProcedure.customSky.add(PREDICATE);
		} catch (Exception e) {
		}
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, double partialTick) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("tardif_mod:space"))) {
			renderStars(1500, 10842, 90, (float) (world.getTimeOfDay((float) partialTick) * 360), 0, 255 << 24 | 255 << 16 | 255 << 8 | 255, true);
		}
	}
}
